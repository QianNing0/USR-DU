...
from track1.data_loss import get_data_loss, FilterHigh
...
class AdaptiveDownsamplingModel(nn.Module):
    ...
    def update_G(self):
        ...
        if self.img_var is None:
            loss_data = get_data_loss(self.img_s, self.img_gen, self.data_loss_type, self.args) * self.args.data_ratio
        else:
            loss_data = get_data_loss(self.img_s, self.img_gen, self.data_loss_type, self.args,
        ...
        if self.args.perceptual_loss == 'VGG19':
            loss_per = self.l1_vgg19(self.img_gen,
                                     core.imresize(self.img_s, scale=1 / int(self.args.scale))) * self.args.per_ratio
            loss_G += loss_per
        elif self.args.perceptual_loss == 'VGG19_Per':
            loss_per = self.l1_vgg19_per(self.img_gen,
                                     core.imresize(self.img_s, scale=1 / int(self.args.scale)), self.per_var) * self.args.per_ratio
            loss_G += loss_per
