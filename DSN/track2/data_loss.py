...
def get_data_loss(img_s, img_s_k, img_gen, data_loss_type, args, var=None):
    ...
    if data_loss_type == 'bic':
    ...
    elif data_loss_type == 'kl_gau_ker':
        filter = FilterLow(recursions=1, stride=1, kernel_size=args.kernel_size, padding=False, gaussian=True).cuda(args.gpu)
        ...
        if var is not None:
            ...
            img_gen = img_gen*255
            img_s_k = img_s_k*255
            u = abs(img_gen - img_s_k)
            return (criterionL1(filter(img_gen), filter(img_s_k)) + torch.mean(torch.mul(s, torch.exp(-torch.div(u, s))) - var - 1))/255
        else:
            img_gen = img_gen * 255
            img_s_k = img_s_k * 255
            return criterionL1(filter(img_gen), filter(img_s_k))/255
    else:
