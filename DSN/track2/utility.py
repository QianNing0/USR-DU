...
def plot_loss(save, loss, loss_name):
    ...
    plt.ylabel('loss')
    plt.grid(True)
    plt.savefig(os.path.join(save, 'down_loss_graph_{}.pdf'.format(loss_name)))
    plt.close(fig)


def plot_score(save, fake_score, real_score):
    assert len(fake_score) == len(real_score)
    axis = np.linspace(1, len(fake_score), len(fake_score))
    fig = plt.figure()

    plt.plot(axis, fake_score, axis, real_score)

    # plt.legend()
    plt.xlabel('epoch')
    plt.ylabel('fake_score(blue), real_score(orange)')
    plt.grid(True)
    plt.savefig(os.path.join(save, 'fake_real_score.pdf'))
    plt.close(fig)


def plot_psnr(save, psnrs, axis):
    assert len(psnrs) == len(axis)
    fig = plt.figure()
    ...
    plt.ylabel('PSNR')
    plt.grid(True)
    plt.savefig(os.path.join(save, 'psnr_graph.pdf'))
    plt.close(fig)
