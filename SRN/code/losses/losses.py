...
class UncertaintyLoss(nn.Module):
    ...
    def forward(self, pred, target, var, weight=None, **kwargs):
        """
        Args:
            pred (Tensor): of shape (N, C, H, W). Predicted tensor.
            target (Tensor): of shape (N, C, H, W). Ground truth tensor.
        ...
        if var is not None:
            s = var + 1
            pred = torch.mul(pred, s)
            target = torch.mul(target, s)
        return self.loss_weight * l1_loss(pred, target, weight, reduction=self.reduction)
...
class LPIPSLoss(nn.Module):
    def __init__(self, loss_weight=1.0, rotations=False, flips=False):
    ...
    def forward(self, x, y):
        ...
        if self.flips:
            ...
            if random.choice([True, False]):
                x = torch.flip(x, (3,))
                y = torch.flip(y, (3,))
        return self.loss_weight * self.loss(x, y), None
