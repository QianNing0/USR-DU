...
class ESRGANModel_FS_GAN(SRGANModel):
    """ESRGAN model for single image super-resolution."""
    def __init__(self, opt):
        super(ESRGANModel_FS_GAN, self).__init__(opt=opt)
        train_opt = opt['FS']
        ...
        if train_opt['type'] == 'wavelet':
            ...
            self.filter_high = FilterHigh(kernel_size=train_opt['kernel_size'], gaussian=True).to(self.device)
        elif train_opt['type'] == 'gau':
            # Gaussian
            self.filter_low, self.filter_high = FilterLow(kernel_size=train_opt['kernel_size'], gaussian=True).to(self.device), FilterHigh(kernel_size=train_opt['kernel_size'], gaussian=True).to(self.device)
            self.fs = self.filter_func
        elif train_opt['type'] == 'avgpool':
            # avgpool
            self.filter_low, self.filter_high = FilterLow(kernel_size=train_opt['kernel_size']).to(self.device), FilterHigh(kernel_size=train_opt['kernel_size']).to(self.device)
            self.fs = self.filter_func
        else:
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.fake_lq_output_low_f, self.fake_lq_output_high_f = self.fs(self.fake_lq_output, norm=self.norm)
        _, self.real_lq_output_high_f = self.fs(self.real_lq_output, norm=self.norm)
        self.real_gt_low_f, self.real_gt_high_f = self.fs(self.real_gt, norm=self.norm)
        _, self.unpair_real_gt_high_f = self.fs(self.unpair_real_gt, norm=self.norm)
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            # gan loss
            fake_fake_lq_g_pred = self.net_d(self.fake_lq_output_high_f)
            ...
            fake_real_lq_g_pred = self.net_d2(self.real_lq_output_high_f)
        ...
        # real
        real_real_gt_d_pred = self.net_d(self.real_gt_high_f)
        l_d_real_gt_real = self.cri_gan(real_real_gt_d_pred, True, is_disc=True)
        l_d_real_gt_real.backward()
        # fake
        fake_fake_lq_d_pred = self.net_d(self.fake_lq_output_high_f.detach().clone())  # clone for pt1.9
        ...
        # real
        real_unpair_real_gt_d_pred = self.net_d2(self.unpair_real_gt_high_f)
        l_d_unpair_real_gt_real = self.cri_gan(real_unpair_real_gt_d_pred, True, is_disc=True)
        l_d_unpair_real_gt_real.backward()
        # fake
        fake_real_lq_d_pred = self.net_d2(self.real_lq_output_high_f.detach().clone())  # clone for pt1.9
