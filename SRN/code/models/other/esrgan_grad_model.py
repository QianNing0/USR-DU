...
def gradient(x):
    if x.size(1) > 1:
        convert = x.new(1, 3, 1, 1)
        convert[0, 0, 0, 0] = 76.544
        convert[0, 1, 0, 0] = 150.072
        convert[0, 2, 0, 0] = 29.184
        x.mul_(convert).div_(256)
...
class ESRGANModel_Grad(SRGANModel):
    """ESRGAN model for single image super-resolution."""
    def __init__(self, opt):
        super(ESRGANModel_Grad, self).__init__(opt=opt)
        train_opt = opt['FS']
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output_grad = gradient(self.output)
        self.gt_grad = gradient(self.gt)
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            # pixel loss
            if self.opt['weighted_loss']:
                l_g_pix = self.UncertaintyLoss(self.output, self.gt, self.var / 255)
                l_g_total += l_g_pix
