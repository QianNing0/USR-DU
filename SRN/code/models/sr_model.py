...
from code.metrics import calculate_metric
from code.utils import get_root_logger, imwrite, tensor2img
from code.utils.registry import MODEL_REGISTRY
from .base_model import BaseModel
from code.utils import USMSharp
...
class SRModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        # pixel loss
        if self.cri_pix:
            l_pix = self.cri_pix(self.output, l1_gt)
            l_total += l_pix
            loss_dict['l_pix'] = l_pix
        # perceptual loss
        if self.cri_perceptual:
            l_percep, l_style = self.cri_perceptual(self.output, percep_gt)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        with_metrics = self.opt['val'].get('metrics', False)
